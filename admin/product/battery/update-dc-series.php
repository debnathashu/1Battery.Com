...
<?php
	if(isset($_POST['Update'])){		
		...
		$BIFileName = $_FILES['BI']['tmp_name'];
		$BIRealName = $_FILES['BI']['name'];
		if($BIRealName)
		{
			if (is_uploaded_file($BIFileName))
			{
				if (file_exists("../../../graphics/product/" . $BIRealName))
				{
					$massage= "<h5 style=\"color:red;font-style: italic;\">Your uploaded image'{$BIRealName}' already exists.</h5>";
				}
				else if(move_uploaded_file($BIFileName, "../../../graphics/product/$BIRealName")) 
				{ 
					$BImage = $connection->query("UPDATE dc_series set Image='{$BIRealName}' where BID='$BID'");// value insert into database
					$dc = $connection->query("UPDATE dc_series set IRefer='{$IRefer}', Volt='{$Volt}', Cap25Min='{$Cap25Min}', Cap75Min='{$Cap75Min}', Cap5HR='{$Cap5HR}', Cap20HR='{$Cap20HR}', Cranking='{$Cranking}' where BID='$BID'");
					...
					$massage= "<h5 style=\"color:green;font-style: italic;\">Add Successfull.</h5>";
					header('Location: ../../product/battery/dc-series.php');
				}						
			}
		}
		else{
...
<title>Update DC Series Battery | 1Battery | LockersITSolution</title>
...
<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
	<div class="row">
		<ol class="breadcrumb">
			...
			<li class="active">Update DC Series Battery</li>
		</ol>
	</div><!--/.row-->
	
	<div class="row">
		<div class="col-lg-12">
			<h1 class="page-header">Change Details<a href="../../product/battery/dc-series.php" class="btn btn-primary pull-right">Back to DC Series</a></h1>
		</div>
	</div><!--/.row-->
					...
					<form role="form" method="post" enctype='multipart/form-data'>
						...
						<div class="col-md-4">
							...
							<div class="form-group">
								...
								<input type="text" class="form-control" value="<?php echo $row_dc_series["BID"]; ?>" readonly="readonly" />
							</div>						
							<div class="form-group">
								<label>Title*</label>
							...
							<div class="form-group">
								...
								<input name="IRefer" id="IRefer" required type="text" class="form-control" value="<?php echo $row_dc_series['IRefer']; ?>" />
							</div>
							<div class="form-group">
								<label>Voltage*</label>
								<input name="Volt" id="Volt" required type="text" class="form-control" value="<?php echo $row_dc_series["Volt"]; ?>" />
							</div>
							<div class="form-group">
								<label>Reserve Capacity(minutes) @ 25 Amps*</label>
								<input name="Cap25Min" id="Cap25Min" required type="text" class="form-control" value="<?php echo $row_dc_series["Cap25Min"]; ?>" />
							</div>
							<div class="form-group">
								<label>Reserve Capacity(minutes) @ 75 Amps*</label>
								<input name="Cap75Min" id="Cap75Min" required type="text" class="form-control" value="<?php echo $row_dc_series["Cap75Min"]; ?>" />
							</div>
							<div class="form-group">
								<label>Capacity(AH) @ 5 HR*</label>
								<input name="Cap5HR" id="Cap5HR" required type="text" class="form-control" value="<?php echo $row_dc_series["Cap5HR"]; ?>" />
							</div>
							<div class="form-group">
								<label>Capacity @ 20 HR*</label>
								<input name="Cap20HR" id="Cap20HR" required type="text" class="form-control" value="<?php echo $row_dc_series["Cap20HR"]; ?>" />
							</div>
							<div class="form-group">
								<label>Cranking/CCA(Amps)*</label>
								<input name="Cranking" id="Cranking" required type="text" class="form-control" value="<?php echo $row_dc_series["Cranking"]; ?>" />
							</div>
							<div class="form-group clearfix">
								<label>Battery Image</label><br>
								<img src="../../../graphics\product\<?php echo $row_dc_series['Image']; ?>" alt="Item Avatar" height="65px"  />	
						...
						<div class="col-md-12">
							...
							<div class="checkbox pull-right">
								<label>
									<input name="BID" id="BID" required type="checkbox" value="<?php echo $row_dc_series["BID"]; ?>">Yes. I want update <?php echo $row_dc_series["BID"]; ?> battery.
								</label>
							</div>
						</div>
