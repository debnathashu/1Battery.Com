...
<?php
	if(isset($_GET['BID'])){
		$BID = $_GET['BID'];
		$full_force = $connection->query("select * from full_force where BID='$BID'");
		$row_full_force = $full_force->fetch_array(MYSQLI_BOTH);
...
<?php
	if(isset($_POST['Update'])){
		...
		// Default
		$Group = 'FF';
		$dc = $connection->query("UPDATE full_force set IRefer='{$IRefer}', Volt='{$Volt}', Cap25Min='{$Cap25Min}', Cap75Min='{$Cap75Min}', Cap5HR='{$Cap5HR}', Cap20HR='{$Cap20HR}', Cranking='{$Cranking}' where BID='$BID'");
		...
		header('Location: ../../product/battery/full-force.php');
	}
...
<title>Update Full Force Battery | 1Battery | LockersITSolution</title>
...
<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
	<div class="row">
		<ol class="breadcrumb">
			...
			<li><a href="../../product/battery/full-force.php" title="Full Force"><svg class="glyph stroked line-graph"><use xlink:href="#stroked-line-graph"></use></svg></a></li>
			<li class="active">Update Full Force Battery</li>
		</ol>
	</div><!--/.row-->
	
	<div class="row">
		<div class="col-lg-12">
			<h1 class="page-header">Change Details<a href="../../product/battery/full-force.php" class="btn btn-primary pull-right">Back to Full Force</a></h1>
		</div>
	</div><!--/.row-->
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-default">
				<div class="panel-heading">Update <?php echo $row_full_force["BID"]; ?> Battery Details</div>
					...
					<form role="form" method="post">
						...
						<div class="col-md-4">
							...
							<div class="form-group">
								...
								<input type="text" class="form-control" value="<?php echo $row_full_force["BID"]; ?>" readonly="readonly" />
							</div>						
							<div class="form-group">
								<label>Title*</label>
							...
							<div class="form-group">
								...
								<input name="IRefer" id="IRefer" required type="text" class="form-control" value="<?php echo $row_full_force['IRefer']; ?>" />
							</div>
							<div class="form-group">
								<label>Voltage*</label>
								<input name="Volt" id="Volt" required type="text" class="form-control" value="<?php echo $row_full_force["Volt"]; ?>" />
							</div>
							<div class="form-group">
								<label>Reserve Capacity(minutes) @ 25 Amps*</label>
								<input name="Cap25Min" id="Cap25Min" required type="text" class="form-control" value="<?php echo $row_full_force["Cap25Min"]; ?>" />
							</div>
							<div class="form-group">
								<label>Reserve Capacity(minutes) @ 75 Amps*</label>
								<input name="Cap75Min" id="Cap75Min" required type="text" class="form-control" value="<?php echo $row_full_force["Cap75Min"]; ?>" />
							</div>
							<div class="form-group">
								<label>Capacity(AH) @ 5 HR*</label>
								<input name="Cap5HR" id="Cap5HR" required type="text" class="form-control" value="<?php echo $row_full_force["Cap5HR"]; ?>" />
							</div>
							<div class="form-group">
								<label>Capacity @ 20 HR*</label>
								<input name="Cap20HR" id="Cap20HR" required type="text" class="form-control" value="<?php echo $row_full_force["Cap20HR"]; ?>" />
							</div>
							<div class="form-group">
								<label>Cranking/CCA(Amps)*</label>
								<input name="Cranking" id="Cranking" required type="text" class="form-control" value="<?php echo $row_full_force["Cranking"]; ?>" />
							</div>
							<div class="form-group">
						...
						<div class="col-md-12">
							...
							<div class="checkbox pull-right">
								<label>
									<input name="BID" id="BID" required type="checkbox" value="<?php echo $row_full_force["BID"]; ?>">Yes. I want update <?php echo $row_full_force["BID"]; ?> battery.
								</label>
							</div>
						</div>
